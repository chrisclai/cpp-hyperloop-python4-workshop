...
import math
from PIL import Image, ImageTk  # pip insetall Pillow
from _thread import *
import threading

if len(sys.argv) != 2:
    print("Usage: python3 client.py <hostIP>")
    sys.exit(1)

serverIP = sys.argv[1]

s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
s.connect((serverIP, 8009))
...
# LABEL
title = tk.Label(main_canv, text="CPP HYPERLOOP", font=('courier new',24,'bold italic'), justify='center', bg='black', fg='#149dca')
title.place(relx=0.5,rely=0.075,anchor='center')

subtitle = tk.Label(main_canv, text="Python IV Workshop", font=('courier new',20,'bold'), justify='center', bg='black', fg='#ca4114')
